        ...
        for i in range(len(points_dict)):
            if points_dict[0]['label'] == 'None':
                ...
                if points_dict[i]['shape_type'] == 'None':
                    ...
                    if points_dict[i]['points'][0][1] > json_labels["imageHeight"] // 2: side = {"bottom": 0, "top": 1}
                    else: side = {"top": 0, "bottom": 1}

                    top_x = points_dict[i]['points'][side["top"]][self.coordinates["x"]]
                    top_y = points_dict[i]['points'][side["top"]][self.coordinates["y"]]
                    bottom_x = points_dict[i]['points'][side["bottom"]][self.coordinates["x"]]
                    bottom_y = points_dict[i]['points'][side["bottom"]][self.coordinates["y"]] \
                    ...
                    left_points_list.append(point_pair[json_labels['lr_to_top_down']["left"]])
                    right_points_list.append(point_pair[json_labels['lr_to_top_down']["right"]])
                    ...
                    if points_dict[i]['points'][0][1] < json_labels["imageHeight"] // 2:
                        if json_labels["lr_to_top_down"]["left"] == "top": left_points_list.append((x, y))
                        else: right_points_list.append((x, y))

                    else:
                        if json_labels["lr_to_top_down"]["left"] == "bottom":
                            left_points_list.append((x, y - json_labels["imageHeight"] // 2))
