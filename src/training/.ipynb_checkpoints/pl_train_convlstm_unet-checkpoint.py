...
import utils
from pl_datamodules import EndoDataModule
from pl_modules import ConvLSTMUnet
import yaml
from models import UNet_pl
from dataloader import SutureDataset
import numpy as np
import random
from pytorch_lightning.utilities.seed import reset_seed, seed_everything
...
def main(args, config_dict):
    ...
    model = ConvLSTMUnet(**model_config, lr=gen_namespace.lr)
    model.init_weights()
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="epoch={epoch:02d}-train_loss={loss/train_loss:.2f}",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr', type=float, default=0.0003901149743289209,
    ...
    parser = ConvLSTMUnet.add_model_specific_args(parser)
    ...
    hrs, min, sec = utils.get_hrs_min_sec(train_duration)
