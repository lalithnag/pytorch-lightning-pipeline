...
def send_training_completion_message(name, num_epochs):
    ...
    """
    message = emoji.emojize('Hi, the training for the experiment "{}" for the suture detection \
    model has completed training for {} epochs :thumbs_up:'.format(name,
                                                                   num_epochs))
    bot = telegram.Bot(token='') # provide your bot token here
    bot.send_message(chat_id=424150566, text=message)
...
def iterify(iterable):  # if there is just one element for ex., converts into iterable and returns
    try:
        iter(iterable)
    except TypeError:
        iterable = [iterable]
...
def iterify_list(iterable):  # if there is just one element for ex., converts into iterable and returns
    if not isinstance(iterable, list):
        iterable = [iterable]
    return iterable


def dict_append_or_create(main_dict, new_dict):
    new_key = list(set(new_dict))[0]
    if new_key in main_dict.keys(): main_dict[new_key].append(new_dict[new_key])
    else: main_dict[new_key] = [new_dict[new_key]]
    return main_dict


def get_non_zero_idx(list_, zero_value=(), element='first'):
    non_zero_indices = [i for i, e in enumerate(list_) if e != zero_value]
    if element=='first': return np.min(non_zero_indices) if non_zero_indices else None
    elif element == 'last': return np.max(non_zero_indices) if non_zero_indices else None
